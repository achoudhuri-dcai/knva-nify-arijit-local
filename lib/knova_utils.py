...
def get_vectorstore(
    ...
    # Get vectorstore
    chroma_database = chromadb.PersistentClient(path=FOLDER_PATH)
...
def query_vectorstore(
        ...
        ,EMBEDDING_MODEL:object=BedrockTitanEmbeddingFunction()     # Embedding model to use. Must be the same one used to create the vectorstore.
    ):
    funcname = inspect.currentframe().f_code.co_name
    ...
    # Retrieve docs
        ...
        for i, collection_i in enumerate(collection_list):    # If there are multiple collections, loop through
            collection = chroma_database.get_collection(name=collection_i.name)
            ...
            if i == 0:
                retrieved_docs_all_collections = retrieved_docs
            else:
                for KEY in retrieved_docs_all_collections.keys():
                    # Make sure value for this key is a list (rather than NoneType)
                    if isinstance(retrieved_docs_all_collections[KEY], list):
                        retrieved_docs_all_collections[KEY] = retrieved_docs_all_collections[KEY] + retrieved_docs[KEY]
